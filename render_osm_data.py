...
def heatmap_objects(grid, h=4, width=10, bar_scale=0.9, numColors=10, colormap=cm.summer):
    # Find maximum value
    #grid = np.log(grid + 1)  # logarithmic scale
    z_max = np.max(grid)
...
if __name__ == '__main__':
    ...
    for frame in range(1, num_frames):
        ...
        camera.location = (x, y, 7)
        camera.keyframe_insert(data_path="location", index=-1, frame=frame)


    # Render result
    frames_folder = '{}_{}_{}_{}_{}_{}'.format(
        iso_a2, tag_key, tag_value, camera_type, res_x, res_y)
